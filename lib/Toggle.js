...
const Helpers = {
  ...
  async toggleCheckbox (page, selector, label = 'toggle', checked = false) {
    const response = await page.$eval(selector, (el, checkValue) => {
      let done = false;
      ...
      return done;
    }, checked).catch(ex => logger.debugAndReturn(false,
      'toggle.checkbox %s for %s error: %s', selector, label, ex));
  ...
  async toggleSelectByText (page, selector, inputValues = [], label = 'toggle', selected = false) {
    ...
    const response = await page.$eval(selector, (el, valueList, selectValue) => {
      ...
      return done;
    }, values, selected).catch(ex => logger.debugAndReturn(false,
      'toggle.select %s for %s error: %s', selector, label, ex));
