...
const toggleCheckbox = async (page, selector, label = 'toggle', checked = false) => {
  const response = await page.$eval(selector, (el, checkValue) => !!angular
    ...
    .trigger('focusout')
    .length, checked)
    .catch(ex => logger.debug('checkbox.toggle %s for %s error: %s', selector, label, ex));
...
const toggleSelectByText = async (page, selector, inputValues = [], label = 'toggle', selected = false) => {
  ...
  const response = await page.$eval(selector, (el, valueList, selectValue) => {
    ...
    .catch(ex => logger.debug('select.toggle %s for %s error: %s', selector, label, ex));
...
const Toggle = {
  ...
  async check (selector, label = 'toggle') {
    return toggleCheckbox(this, selector, label, true);
  },
  /**
  ...
  async deselectByText (selector, values, label = 'toggle') {
    return toggleSelectByText(this, selector, values, label, false);
  },
  /**
  ...
  async selectByText (selector, values, label = 'toggle') {
    return toggleSelectByText(this, selector, values, label, true);
  },
  /**
  ...
  async uncheck (selector, label = 'toggle') {
    return toggleCheckbox(this, selector, label, false);
  },
};
