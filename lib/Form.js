...
const getValue = (data, pattern, defaultValue = null) => {
  const result = jmespath(data, pattern);
  return typeof (result) !== 'undefined' && result !== null ? result : defaultValue;
};

const fillOutField = (page, config, data = {}) => new Promise(async (resolve) => {
  let values = null;

  if (['deselect-text', 'select-text', 'type'].includes(config.type)) {
    values = getValue(data, config.value, config.defaultValue);
  }
  ...
  async fillOut (configs, data = {}) {
    if (Array.isArray(configs)) {
      return new Promise(async (resolve) => {
        await configs.reduce((vow, config) => vow.then(() => fillOutField(this, config, data)),
          Promise.resolve());
        resolve();
      });
    }
